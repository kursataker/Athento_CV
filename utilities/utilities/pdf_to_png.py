...
"""
    ...
    - **pdf_to_png**: set of functions that check if the input file has a PDF
    extension (get_pdf function). If so, it transforms each page of the PDF file
    into a PNG image (transform).
...
def transform(input_file, dpi = 400):
    try:
        ...
        cmd = "pdftoppm -png -rx {0} -ry {0} {1} {2}".format(dpi, input_file,
                                                                output_name)
        os.popen(cmd)
        print "Done."
